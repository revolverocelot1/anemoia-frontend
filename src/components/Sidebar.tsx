...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onToggle }) => {
  ...
  React.useEffect(() => {
    // This is a bulletproof polyfill for broken position:fixed containing blocks.
    // By strictly using absolute positioning and mapping top to scrollY, 
    // we bypass the browser's broken rendering logic entirely.
    const syncPosition = () => {
      const scrollY = window.scrollY || document.documentElement.scrollTop;
      if (sidebarRef.current) {
        sidebarRef.current.style.top = `${scrollY}px`;
      }
      if (toggleRef.current) {
        toggleRef.current.style.top = `${scrollY + 20}px`; // 1.25rem = 20px
      }
  ...
  const sidebarContent = (
    <>
      {/* Toggle Button Wrapper — Bulletproof absolute positioning sync */}
      <div 
        ref={toggleRef}
      ...
      >
        <motion.button
          onClick={onToggle}
          className="w-11 h-11 bg-black/80 backdrop-blur-md rounded-xl shadow-lg border border-cyan-500/30 flex items-center justify-center hover:bg-gray-900/90 hover:border-cyan-400/60 transition-all pointer-events-auto"
      ...
      <div 
        ref={overlayRef}
        className="absolute w-full h-screen left-0 z-[999998] pointer-events-none"
        style={{ top: 0 }}
      >
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="w-full h-full bg-black/40 backdrop-blur-[2px] pointer-events-auto"
              onClick={onToggle}
            />
          )}
        </AnimatePresence>
      ...
      <div 
        ref={sidebarRef}
        className="absolute left-0 z-[999999] pointer-events-none w-0 h-screen"
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="w-96 max-w-[85vw] h-screen flex flex-col bg-gray-950/98 backdrop-blur-xl shadow-2xl border-r border-cyan-500/15 pointer-events-auto"
            >
              {/* Header — sticky top */}
