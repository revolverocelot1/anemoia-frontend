...
import {
  ...
  Mouse, Keyboard, MonitorSmartphone, X, Video, Move3d
} from 'lucide-react';
...
interface CameraAPI {
  ...
  resetLens: () => void;
  getTelemetry: () => CameraTelemetry | null;
}
...
type GaussianViewerCalibration = SplatLensMetadata;
      ...
      getTelemetry: () => {
        ...
        return {
          ...
          target: orbitControlsRef.current
            ? [orbitControlsRef.current.target.x, orbitControlsRef.current.target.y, orbitControlsRef.current.target.z]
            : [0, 0, 0],
          alpha: 0,
          beta: 0,
          radius: camera.position.length(),
          horizontalFov: perspectiveCamera.isPerspectiveCamera ? perspectiveCamera.fov : 60,
          focalPx: 0,
        };
      },
    };
...
const GaussianSplatRenderer = ({
  ...
  format?: string;
  cameraAPIRef?: React.MutableRefObject<CameraAPI | null>;
  calibration?: GaussianViewerCalibration;
  onTelemetryChange?: (telemetry: CameraTelemetry | null) => void;
  ...
  // Current horizontal FOV â€” the single source of truth.
  // Initialised from metadata intrinsics on model load.
  const currentFovRef = useRef(60);
  const defaultFovRef = useRef(60);

  const viewStateRef = useRef({
    alpha: 0,
    beta: calibration?.viewerCalibration?.frontBeta ?? 0,
    radius: 3,
    target: (calibration?.viewerCalibration?.center ?? [0, 0, 0]) as [number, number, number],
  });

  // â”€â”€ FOV helpers â”€â”€

  /** Stamp the current FOV onto gsplat camera data. */
  const stampFov = useCallback((fov: number) => {
    if (!cameraObjRef.current || !canvasRef.current) return;
    ...
    const fovRad = (Math.max(5, Math.min(170, fov)) * Math.PI) / 180;
  ...
  /** Emit camera telemetry to the parent. */
  const emitTelemetry = useCallback(() => {
    if (!cameraObjRef.current) return;
    const camera = cameraObjRef.current;
    const data = camera.data;
    const horizontalFov = (2 * Math.atan((data.width || 1) / (2 * data.fx)) * 180) / Math.PI;

    const rotation = SPLAT.Matrix3.RotationFromQuaternion(camera.rotation).buffer;
    const forward = new SPLAT.Vector3(-rotation[2], -rotation[5], -rotation[8]).normalize();
    const focusDepth = calibration?.viewerCalibration?.focusDepth ?? calibration?.viewerCalibration?.center?.[2] ?? 0;
    let liveTarget = [...viewStateRef.current.target] as [number, number, number];
    if (Math.abs(forward.z) > 1e-4) {
      const t = (focusDepth - camera.position.z) / forward.z;
      liveTarget = [
        camera.position.x + forward.x * t,
        camera.position.y + forward.y * t,
        focusDepth,
      ];
    }
    const dx = liveTarget[0] - camera.position.x;
    const dy = liveTarget[1] - camera.position.y;
    const dz = liveTarget[2] - camera.position.z;
    const radius = Math.sqrt(dx * dx + dy * dy + dz * dz);
    const alpha = -Math.atan2(dx, dz);
    const beta = Math.atan2(dy, Math.sqrt(dx * dx + dz * dz));
  ...
  useEffect(() => {
    ...
    // Compute metadata-driven FOV
    const metadataFov = computeIntrinsicFov(calibration, canvas.width, canvas.height, 60);
    defaultFovRef.current = metadataFov;
    currentFovRef.current = metadataFov;
    ...
    const resizeCanvas = () => {
      ...
      if (parent) {
        ...
        if (cameraObjRef.current) {
          cameraObjRef.current.data.setSize(width, height);
          stampFov(currentFovRef.current);
        }
      }
    };
    resizeCanvas();
    ...
    const center = getCalibrationCenter(calibration);
    const isCameraSpace = calibration?.viewerCalibration?.cameraSpace === true;
    const frontBeta = getFrontBeta(calibration);
    const parallaxBeta = getParallaxBeta(calibration);

    const createControls = (
      alpha: number,
      beta: number,
      radius: number,
      target: [number, number, number] = viewStateRef.current.target
    ) => {
      controlsObjRef.current?.dispose();
      viewStateRef.current = { alpha, beta, radius, target };

      const controls = new SPLAT.OrbitControls(
        camera, canvas,
        alpha, beta, radius,
        false, new SPLAT.Vector3(target[0], target[1], target[2])
      );
      controls.setCameraTarget?.(new SPLAT.Vector3(target[0], target[1], target[2]));

      // Radia Galleryâ€“inspired control parameters
      const fovScale = Math.max(0.05, Math.min(2.0, currentFovRef.current / defaultFovRef.current));
      controls.orbitSpeed = 1.5 * fovScale;
      controls.panSpeed = 1.0 * fovScale;
      controls.zoomSpeed = 2.0;
      controls.dampening = 0.08; // Radia uses 0.08
      controls.minZoom = Math.max(0.3, radius * 0.12);
      controls.maxZoom = Math.max(15, radius * 3.0);
      controlsObjRef.current = controls;

      // Kick-start dampening convergence
      controls.update();
      controls.update();
      controls.update();
      ...
      canvas.dispatchEvent(new MouseEvent('mousedown', { clientX: cx, clientY: cy, button: 0, bubbles: false }));
      canvas.dispatchEvent(new MouseEvent('mousemove', { clientX: cx + 0.05, clientY: cy, button: 0, bubbles: false }));
      window.dispatchEvent(new MouseEvent('mouseup', { clientX: cx + 0.05, clientY: cy, button: 0, bubbles: false }));
    ...
    const getInitialConfig = () => {
      if (isCameraSpace) {
        return computeCameraSpaceTarget(calibration, frontBeta);
      }
      return {
        radius: computeFitRadius(calibration, metadataFov, frontBeta, canvas.width, canvas.height, 5),
        target: center,
      };
    };

    const initialConfig = getInitialConfig();
    createControls(0, frontBeta, initialConfig.radius, initialConfig.target);
    ...
    if (cameraAPIRef) {
      cameraAPIRef.current = {
        resetView: () => {
          ...
          createControls(0, frontBeta, cfg.radius, cfg.target);
          stampFov(defaultFovRef.current);
          emitTelemetry();
        },
        frontView: () => {
          if (isCameraSpace) {
            const cfg = computeCameraSpaceTarget(calibration, frontBeta);
            createControls(0, frontBeta, cfg.radius, cfg.target);
          } else {
            createControls(0, frontBeta, computeFitRadius(calibration, currentFovRef.current, frontBeta, canvas.width, canvas.height), center);
          }
          emitTelemetry();
        },
        parallaxView: () => {
          if (isCameraSpace) {
            const cfg = computeCameraSpaceTarget(calibration, parallaxBeta);
            createControls(0, parallaxBeta, cfg.radius, cfg.target);
          } else {
            createControls(0, parallaxBeta, computeFitRadius(calibration, currentFovRef.current, parallaxBeta, canvas.width, canvas.height), center);
          }
          emitTelemetry();
        },
        sideView: () => {
          createControls(-Math.PI / 2, frontBeta, computeFitRadius(calibration, currentFovRef.current, frontBeta, canvas.width, canvas.height), center);
          emitTelemetry();
        },
        topView: () => {
          createControls(0, -(Math.PI / 2 - 0.01), computeFitRadius(calibration, currentFovRef.current, 0, canvas.width, canvas.height) * 1.15, center);
          emitTelemetry();
        },
        setAutoRotate: (enabled: boolean) => {
          autoRotateRef.current = enabled;
          if (!enabled && autoRotateState.active) {
        ...
        getTelemetry: () => {
          if (!cameraObjRef.current) return null;
          const data = cameraObjRef.current.data;
          const horizontalFov = (2 * Math.atan((data.width || 1) / (2 * data.fx)) * 180) / Math.PI;
          return {
            position: [cameraObjRef.current.position.x, cameraObjRef.current.position.y, cameraObjRef.current.position.z],
            target: [...viewStateRef.current.target] as [number, number, number],
            alpha: viewStateRef.current.alpha,
            beta: viewStateRef.current.beta,
            radius: viewStateRef.current.radius,
            horizontalFov,
            focalPx: data.fx,
          };
        },
    ...
    // â”€â”€ Auto-rotate state â”€â”€
    const autoRotateState = { active: false, lastX: 0, lastY: 0 };

    // â”€â”€ Keyboard shortcuts â”€â”€
    const simulateDrag = (dx: number, dy: number) => {
      const rect = canvas.getBoundingClientRect();
      const cx = rect.left + rect.width / 2;
      const cy = rect.top + rect.height / 2;
      canvas.dispatchEvent(new MouseEvent('mousedown', { clientX: cx, clientY: cy, button: 0, bubbles: false }));
      canvas.dispatchEvent(new MouseEvent('mousemove', { clientX: cx + dx, clientY: cy + dy, button: 0, bubbles: false }));
      window.dispatchEvent(new MouseEvent('mouseup', { clientX: cx + dx, clientY: cy + dy, button: 0, bubbles: false }));
    };
    const simulateZoom = (deltaY: number) => {
      canvas.dispatchEvent(new WheelEvent('wheel', { deltaY, bubbles: false, cancelable: true }));
    };

    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.target instanceof HTMLInputElement || e.target instanceof HTMLTextAreaElement) return;
      const key = e.key.toLowerCase();
      if (['arrowleft', 'arrowright', 'arrowup', 'arrowdown', 'w', 'a', 's', 'd', 'q', 'e', 'r', '1', '2', '3', '7', 'home'].includes(key)) {
        e.preventDefault();
      }
      switch (key) {
        case 'arrowleft': case 'a': simulateDrag(-24, 0); break;
        case 'arrowright': case 'd': simulateDrag(24, 0); break;
        case 'arrowup': case 'q': simulateDrag(0, -18); break;
        case 'arrowdown': case 'e': simulateDrag(0, 18); break;
        case 'w': simulateZoom(-120); break;
        case 's': simulateZoom(120); break;
        case 'r': case 'home': cameraAPIRef?.current?.resetView(); break;
        case '1': cameraAPIRef?.current?.frontView(); break;
        case '2': cameraAPIRef?.current?.parallaxView(); break;
        case '3': cameraAPIRef?.current?.sideView(); break;
        case '7': cameraAPIRef?.current?.topView(); break;
      }
    };
    window.addEventListener('keydown', handleKeyDown);
    ...
    const animate = () => {
      // Auto-rotate via synthetic drag
      ...
      // Kick-start dampening (SwiftShader workaround)
      ...
      // Ensure FOV stays correct (guard against gsplat internals overwriting fx/fy)
      if (canvas.width > 0 && camera.data) {
        const desiredFovRad = (currentFovRef.current * Math.PI) / 180;
        const expectedFx = (canvas.width / 2) / Math.tan(desiredFovRad / 2);
        if (Math.abs(camera.data.fx - expectedFx) > 0.5 || Math.abs(camera.data.fy - expectedFx) > 0.5) {
          camera.data.fx = expectedFx;
          camera.data.fy = expectedFx;
        }
      }

      controlsObjRef.current?.update();
      // Emit telemetry at ~8Hz (not every frame)
      if (frameCount % 8 === 0) emitTelemetry();
    ...
    (async () => {
      try {
        ...
        // Set camera intrinsics from metadata
        camera.data.setSize(canvas.width, canvas.height);
        stampFov(metadataFov);
          ...
          <section>
            ...
            {isGaussian ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <HelpItem keys="A / D / â† / â†’" action="Orbit left / right in Gaussian viewer" />
                <HelpItem keys="Q / E / â†‘ / â†“" action="Tilt up / down in Gaussian viewer" />
                <HelpItem keys="W / S" action="Zoom in / out in Gaussian viewer" />
                <HelpItem keys="1 / 2 / 3 / 7" action="Front / depth / side / top presets" />
                <HelpItem keys="R / Home" action="Reset camera and restore original framing" />
                <HelpItem keys="H / ?" action="Toggle settings / help" />
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    <HelpItem keys="â†‘ â†“ â† â†’" action="Orbit camera around the target" />
          ...
          <section>
            ...
            <div className="space-y-3">
              ...
              <HelpExplanation
                title="Wireframe"
                desc={isGaussian
                  ? "Not available for Gaussian Splats. Gaussian splats are rendered as projected 2D Gaussians â€” there are no polygon edges to visualize."
    ...
    >
      <div className="flex items-center gap-1 px-3 py-2 bg-gray-900/90 backdrop-blur-xl rounded-2xl 
        ...
        <div className="flex items-center gap-1 pr-2 border-r border-gray-700/50">
          ...
          />
          {viewerType === 'gaussian' && onParallaxView && (
            <ToolbarButton
              icon={<Move3d className="w-4 h-4" />}
              label="Depth"
              onClick={onParallaxView}
              tooltip="Slight calibrated parallax view (Numpad 2)"
            />
          )}
          <ToolbarButton
        ...
        <div className="flex items-center gap-1">
          {viewerType === 'gaussian' && onToggleRecording && (
            <ToolbarButton
              icon={<Video className={`w-4 h-4 ${isRecording ? 'text-red-300' : ''}`} />}
              label={recordBusy ? 'Busy' : isRecording ? 'Stop' : 'Record'}
              onClick={onToggleRecording}
              active={isRecording}
              tooltip="Record viewport to MP4"
            />
          )}
          {onToggleMonitor && (
            <ToolbarButton
              ...
              active={showMonitor}
              tooltip="Toggle performance + camera monitor"
            />
          )}
          <ToolbarButton
...
const UnifiedRenderer = ({
  ...
  gaussianMetadata?: GaussianViewerCalibration;
}) => {
  const { settings, update } = useViewerSettings();
  ...
  const defaultViewFov = useMemo(
    () => (fileType === 'gaussian' && gaussianMetadata ? computeIntrinsicFov(gaussianMetadata, window.innerWidth, window.innerHeight, 60) : settings.fov ?? 60),
    [fileType, gaussianMetadata, settings.fov],
  );
  const appliedModelLensRef = useRef<string | null>(null);

  useEffect(() => {
    if (fileType !== 'gaussian' || !fileUrl) return;
    // Use metadata-driven FOV for the UI slider/settings
    const lensFov = gaussianMetadata ? computeIntrinsicFov(gaussianMetadata, window.innerWidth, window.innerHeight, 60) : 60;
    const nextKey = `${fileUrl}|${lensFov.toFixed(2)}`;
    if (appliedModelLensRef.current === nextKey) return;
    appliedModelLensRef.current = nextKey;
    update({ fov: lensFov });
  }, [fileType, fileUrl, gaussianMetadata, update]);
  ...
  // FOV sync â€” only forward to camera when user actually changes the slider
  ...
  useEffect(() => {
    ...
    // Only call setFov if we actually have a camera and the value differs
    if (cameraAPIRef.current) {
      const tel = cameraAPIRef.current.getTelemetry?.();
      const currentCameraFov = tel?.horizontalFov;
      // Skip if the camera already shows this FOV (prevents re-entry loops)
      if (currentCameraFov !== undefined && Math.abs(currentCameraFov - settings.fov) < 0.1) return;
      cameraAPIRef.current.setFov(settings.fov);
    }
  }, [settings.fov]);

  const handleToggleRecording = useCallback(async () => {
    if (!viewerCanvas || fileType !== 'gaussian') return;
    ...
    setRecordBusy(true);
    const chunks: Blob[] = [];
    const stream = viewerCanvas.captureStream(30);
    ...
    const recorder = new MediaRecorder(stream, { mimeType: selectedMimeType, videoBitsPerSecond: 8_000_000 });
    ...
    recorder.onstop = async () => {
      try {
        ...
        if (!selectedMimeType.includes('mp4')) {
          ...
          const inputFile = new File([rawBlob], 'viewer-recording.webm', { type: rawBlob.type || 'video/webm' });
          mp4Blob = await ffmpegServiceRef.current.convertVideo(inputFile, 'mp4', {
            codec: 'libx264',
            fps: 30,
          });
        }
        ...
        link.download = `splat-viewer-${Date.now()}.mp4`;
        link.click();
        URL.revokeObjectURL(downloadUrl);
      } catch (error) {
        console.error('[SplatViewer] MP4 recording failed:', error);
      } finally {
    ...
    setIsRecording(true);
    setRecordBusy(false);
  }, [fileType, isRecording, viewerCanvas]);
  ...
  // â”€â”€ Gaussian Splat Viewer â”€â”€
  if (fileType === 'gaussian') {
    return (
      <div className="relative w-full h-full overflow-hidden">
        {/* Canvas layer â€” lowest z so UI overlays always receive clicks */}
        <div className="absolute inset-0 z-0">
          <GaussianSplatRenderer
            url={fileUrl}
            format=".ply"
            cameraAPIRef={cameraAPIRef}
            calibration={gaussianMetadata}
        ...
        <HolographicStats
          visible={showMonitor}
          extraLines={cameraTelemetry ? [
            `Pos ${cameraTelemetry.position.map((v) => v.toFixed(1)).join(', ')}`,
            `Target ${cameraTelemetry.target.map((v) => v.toFixed(1)).join(', ')}`,
            `A/B ${cameraTelemetry.alpha.toFixed(2)} / ${cameraTelemetry.beta.toFixed(2)}`,
            `Lens ${cameraTelemetry.horizontalFov.toFixed(1)}Â° Â· ${Math.round(cameraTelemetry.focalPx)}px`,
          ] : []}
        />
        ...
        <ViewerToolbar
          ...
          viewerType={fileType}
          onParallaxView={() => cameraAPIRef.current?.parallaxView()}
          onToggleRecording={handleToggleRecording}
          isRecording={isRecording}
          recordBusy={recordBusy}
          onToggleMonitor={() => setShowMonitor((prev) => !prev)}
          showMonitor={showMonitor}
        />

        {/* Settings Sidebar â€” z-50 so it always sits above the canvas and toolbar */}
        <div className="absolute right-0 top-0 h-full z-50 pointer-events-none">
          <SplatViewerControls
            ...
            onToggleAutoRotate={handleToggleAutoRotate}
            autoRotate={autoRotate}
            defaultFov={defaultViewFov}
            originalFov={gaussianMetadata?.originalFov}
            sourceWidth={gaussianMetadata?.width}
            sourceHeight={gaussianMetadata?.height}
            currentTelemetry={cameraTelemetry}
            onResetLens={() => cameraAPIRef.current?.resetLens()}
          />
        </div>
  ...
  // â”€â”€ Three.js Renderer (Triangle Splat / Mesh) â”€â”€
  return (
    <div className="relative w-full h-full overflow-hidden">
      <Canvas
      ...
      <HolographicStats visible={showMonitor} />
...
const SplatViewerPage: React.FC = () => {
  ...
  const [gaussianMetadata, setGaussianMetadata] = useState<GaussianViewerCalibration | undefined>(undefined);
  const [error, setError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (loadId && loadedStoreIdRef.current !== loadId) {
      loadedStoreIdRef.current = loadId;
        ...
        .then((storedFile) => {
          if (storedFile) {
            ...
            if (storedFile.metadata) {
              const computedOriginalFov = typeof storedFile.metadata.originalFov === 'number'
                ? storedFile.metadata.originalFov
                : (storedFile.metadata.focalLength && storedFile.metadata.width)
                  ? (2 * Math.atan(storedFile.metadata.width / (2 * storedFile.metadata.focalLength)) * 180) / Math.PI
                  : 60;
              setGaussianMetadata({
                defaultFov: storedFile.metadata.defaultFov,
                focalLength: storedFile.metadata.focalLength,
                width: storedFile.metadata.width,
                height: storedFile.metadata.height,
                originalFov: computedOriginalFov,
                viewerCalibration: storedFile.metadata.viewerCalibration,
              });
              setStats({
                splatCount: storedFile.metadata.gaussianCount,
                focalLength: storedFile.metadata.focalLength,
                originalFov: computedOriginalFov,
                processingTimeMs: storedFile.metadata.processingTimeMs,
              });
            }
          } else {
                  ...
                  {gaussianMetadata && (
                    <div className="mt-4 pt-4 border-t border-cyan-500/10">
                      <p className="text-xs font-semibold text-cyan-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                        ...
                        SHARP Generation Info
                      </p>
                      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                        {gaussianMetadata.width && gaussianMetadata.height && (
                          <div>
                            <p className="text-gray-500 text-xs mb-0.5">Source Image</p>
                            <p className="text-sm font-medium text-gray-300">{gaussianMetadata.width}Ã—{gaussianMetadata.height}px</p>
                          </div>
                        )}
                        {typeof gaussianMetadata.originalFov === 'number' && (
                          <div>
                            <p className="text-gray-500 text-xs mb-0.5">Original FOV</p>
                            <p className="text-sm font-medium text-gray-300">{Math.round(gaussianMetadata.originalFov)}Â°</p>
                          </div>
                        )}
                        {typeof gaussianMetadata.defaultFov === 'number' && (
                          <div>
                            <p className="text-gray-500 text-xs mb-0.5">Viewer FOV</p>
                            <p className="text-sm font-medium text-gray-300">{gaussianMetadata.defaultFov.toFixed(1)}Â°</p>
                          </div>
                        )}
                        {typeof gaussianMetadata.focalLength === 'number' && (
                          <div>
                            <p className="text-gray-500 text-xs mb-0.5">Focal Length</p>
                            <p className="text-sm font-medium text-gray-300">{Math.round(gaussianMetadata.focalLength)}px</p>
                          </div>
                        )}
                        {typeof stats.processingTimeMs === 'number' && (
                          <div>
                            <p className="text-gray-500 text-xs mb-0.5">Processing Time</p>
                            <p className="text-sm font-medium text-gray-300">{stats.processingTimeMs >= 1000 ? `${(stats.processingTimeMs / 1000).toFixed(1)}s` : `${stats.processingTimeMs}ms`}</p>
                          </div>
                        )}
                        {gaussianMetadata.viewerCalibration && (
                          <div>
                            <p className="text-gray-500 text-xs mb-0.5">Calibration</p>
                            <p className="text-sm font-medium text-emerald-400 flex items-center gap-1">
                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 inline-block" />
                              {gaussianMetadata.viewerCalibration.cameraSpace ? 'Camera-space' : 'Scene-space'}
                            </p>
                          </div>
                        )}
                      </div>
