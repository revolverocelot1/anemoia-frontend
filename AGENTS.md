...
- The AI upscaler (`src/workers/upscaler.worker.ts`) loads real TF.js GraphModel files from `public/models/` (Real-CUGAN 2x/4x, Real-ESRGAN Anime/General 4x). Model files are in `.gitignore` — download from the xororz/web-realesrgan v0.1.0 GitHub release and extract to `public/models/`. Models are cached in IndexedDB after first download.
